...
Wowser 的 Blog 就这么开通了。

[跳过废话，直接看技术实现 ](#build) 



2015 年，Wowser 总算有个地方可以好好写点东西了。


作为一个 程序猿，其实早就有写 Blog 的想法了。**但是既然作为一个 程序猿…** Blog 这种轮子要是挂在大众博客程序上就太没意思了。一是觉得大部分 Blog 服务都太丑，二是觉得不能随便定制不好玩（可是又懒）……结果就一直连个写 Blog 的地儿都没有。
...
* 非常熟悉的 Git Flow ，**Git Commit 即 Blog Post**
	* 前端最爱啊，Terminal(Z-Shell) + Git 玩起来太爽了
* **Markdown** 写作环境
...
Theme 的 CSS 是基于 Bootstrap 定制的，看得不爽的地方直接在 Less 里改就好了（平时更习惯 Scss ），所以就经常要为了体验补 CSS
